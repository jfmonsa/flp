  ...
  (lambda (exp env) ;expression & enviroment
    (cases expression exp
      ...
      (let-exp (ids rands body)
               (let
                   (
                    (lvalues (map (lambda (x) (direct-target (evaluate-expression x env))) rands))
                    )
      ...
      (set-exp (id exp)
               (begin
                 (setref!
                  ...
                  (evaluate-expression exp env))
                 1 ;Se devuelve 1 para indicar que se realizo correctamente
                 ))
)))
;Enviroments
...
(define deref
  (lambda (ref)
    (cases target (primitive-deref ref)
      ...
      (indirect-target (refi)
          (cases target (primitive-deref refi)
            (direct-target (expi) expi) ;Si tiene un valor expresado lo devuelve
            (indirect-target (p) (eopl:error "La referencia no es valida" refi)))))))
...
(define setref!
  (lambda (ref val)
    (let
        (
         (ref1 (cases target (primitive-deref ref)
                 ;Si es un target directo, modificamos la misma referencia
                 (direct-target (exp) ref)
                 ;Modificamos la referencia interna, si es indirecto
                 (indirect-target (ref-2) ref-2)))
         )
         ...
         (cases reference x
           (a-ref (pos vec)
                  (cases target (vector-ref vec pos)
                    ...
                    (indirect-target (v) #f)))))))
;Paso por referencia
;Paso por referencia aplicación de procedimientos
(define eval-rand
  (lambda (exp env)
    (cases expression exp
      (var-exp (id)
      ...
      (else
       (direct-target (evaluate-expression exp env))))))
